import { Body, Controller, Delete, Get, Param, ParseIntPipe, Post } from '@nestjs/common';
import { PriviligesService } from './priviliges.service';
import { CreatePriviligesDTO } from './DTO/CreatePriviligesDTO';

@Controller('Priviliges')
export class PriviligesController {
  constructor(private readonly appService: PriviligesService) {}

  @Get()
  getAll() {
    return this.appService.getAll();
  }

  @Get('name/:name')
  async getByName(@Param('name') name: string) {
    return this.appService.getByName(name);
  }

  @Get('type/:type')
  async getByType(@Param('type') type: string) {
    return this.appService.getByType(type);
  }

  @Post()
  //async create(@Body() createDto: CreatePriviligesDTO) {
  //  return this.appService.createPriviliges(createDto);
  //}

  @Delete(':id')
  async delete(@Param('id', ParseIntPipe) id: number) {
    return this.appService.deletePriviliges(id);
  }
}
